...
def get_scenario_data(scenario_type: str) -> dict:
    ...
    scenarios = {
        'emergency': {
            'start': 0,  # Central station
            'targets': [15],  # Most critical station (East)
            'constraints': {
        ...
        'maintenance': {
            'start': 4,  # North station
            'targets': [7, 13, 9],  # Stations to visit in order
            'constraints': {
                'visit_order': True,
                'max_total_distance': 2000
            },
            'description': 'Planned maintenance route for 3 stations'
        },
        'balancing': {
            'start': 9,  # South station
            'targets': [7, 11, 16, 19],  # Low energy stations
            'constraints': {
                'min_energy_level': 50,
                'max_total_distance': 1500
            },
